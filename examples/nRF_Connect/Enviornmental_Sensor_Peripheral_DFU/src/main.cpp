...
int main(void) {
    ...
    // ---- 7) Optional DFU feature — SMP service + OTA with rollback ----
    dfu = new HMS_DFU();
    if (dfu->init(ble) != HMS_DFU_STATUS_SUCCESS ||
        dfu->start()  != HMS_DFU_STATUS_SUCCESS) {
    ...
    while (1) {
        ...
        if (now - last >= (int64_t)notifyInterval * 1000) {
            last = now;
