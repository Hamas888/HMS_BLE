...
static void onWrite(const char *svc, const char *chr, const uint8_t *data, size_t len, const uint8_t *mac) {
    ...
    if (strcmp(chr, CFG_INTERVAL) == 0 && len >= sizeof(uint16_t)) {
        notifyInterval = data[0] | ((uint16_t)data[1] << 8);
...
void setup() {
    Serial.begin(115200);
    delay(1000);
    logger.info("=== HMS_BLE Multi-Service Demo (ESP32) ===");
    ...
    if (st != HMS_BLE_STATUS_SUCCESS) {
        logger.error("addService(Environment) failed: %d", st); return;
    }

    st = ble->addCharacteristicToService(ENV_SVC, &tempCh);
    if (st != HMS_BLE_STATUS_SUCCESS) {
        logger.error("addChar(Temperature) failed: %d", st); return;
    }

    st = ble->addCharacteristicToService(ENV_SVC, &humCh);
    if (st != HMS_BLE_STATUS_SUCCESS) {
        logger.error("addChar(Humidity) failed: %d", st); return;
    }

    st = ble->addService(&battSvc);
    if (st != HMS_BLE_STATUS_SUCCESS) {
        logger.error("addService(Battery) failed: %d", st); return;
    }

    st = ble->addCharacteristicToService(BATT_SVC, &battCh);
    if (st != HMS_BLE_STATUS_SUCCESS) {
        logger.error("addChar(Battery Level) failed: %d", st); return;
    }

    st = ble->addService(&cfgSvc);
    if (st != HMS_BLE_STATUS_SUCCESS) {
        logger.error("addService(Config) failed: %d", st); return;
    }

    st = ble->addCharacteristicToService(CFG_SVC, &cfgCh);
    if (st != HMS_BLE_STATUS_SUCCESS) {
        logger.error("addChar(Interval) failed: %d", st); return;
    }
...
void loop() {
    ...
    if (now - last >= (unsigned long)notifyInterval * 1000) {
        last = now;
